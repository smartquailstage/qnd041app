...
from .models import PaaSOrder, PaaSOrderItem
...
def order_detail(obj):
    return mark_safe('<a href="{}">View</a>'.format(
        reverse('paas_orders:admin_order_detail', args=[obj.id])))
...
def order_pdf(obj):
    return mark_safe('<a href="{}">PDF</a>'.format(
        reverse('paas_orders:admin_order_pdf', args=[obj.id])))
...
def order_ebook(obj):
    return mark_safe('<a href="{}" target="_blank">eBook PDF</a>'.format(
        reverse('paas_orders:admin_ebook_pdf', args=[obj.id])
    ))
...
@register_component
class DistribucionSemanalPaaSOrdenesComponent(BaseComponent):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Obtener fechas
        fechas = list(PaaSOrder.objects.values_list("created", flat=True).order_by("created"))
        if not fechas:
            return {
                "title": self.name,
                "grafico_base64": None,
...
@register_component
class OrderPaaSDetailComponent(BaseComponent):
...
def contract_ip(obj):
    return mark_safe('<a href="{}">Contrato PI</a>'.format(
        reverse('paas_orders:admin_contract_ip_pdf', args=[obj.id])
    ))
...
def contract_development(obj):
    return mark_safe('<a href="{}">Contrato Desarrollo</a>'.format(
        reverse('paas_orders:admin_contract_development_pdf', args=[obj.id])
    ))
...
def contract_cloud(obj):
    return mark_safe('<a href="{}">Contrato Cloud</a>'.format(
        reverse('paas_orders:admin_contract_cloud_pdf', args=[obj.id])
    ))
contract_cloud.short_description = "Alquiler Cloud"



@admin.register(PaaSOrder)
class PaaSOrderAdmin(ModelAdmin):
    # Componentes Unfold
    list_sections = [OrderPaaSDetailComponent, DistribucionSemanalPaaSOrdenesComponent]
