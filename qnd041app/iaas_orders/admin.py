...
from .models import IaaSOrder, IaaSOrderItem
...
@register_component
class DistribucionSemanalIaaSOrdenesComponent(BaseComponent):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Obtener fechas
        fechas = list(IaaSOrder.objects.values_list("created", flat=True).order_by("created"))
        if not fechas:
            return {
                "title": self.name,
                "grafico_base64": None,
...
@register_component
class OrderIaaSDetailComponent(BaseComponent):
...
@admin.register(IaaSOrder)
class IaaSOrderAdmin(ModelAdmin):
    # Componentes Unfold
    list_sections = [OrderIaaSDetailComponent, DistribucionSemanalIaaSOrdenesComponent]
    ...
    fieldsets = (
        ...
        ('Pagos y Descuentos', {
            'fields': (
                'paid', 'force_paid', 'braintree_id', 'coupon', 'count'
            ),
